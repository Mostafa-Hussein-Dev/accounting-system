...
// Shared money maths for sales documents (invoices, credit notes). Amounts are
// kept in the document currency and in base (USD); rate is "currency units per
// 1 USD", so base = original / rate (project-wide convention). Mirrors
// purchasing.money.ts but the line carries a unit PRICE and a line discount %.
...
/** Net / VAT / total for one sales line, in the document currency (§21.1). */
export function computeLine(line: LineInput): LineAmounts {
  const gross = line.qty * line.unitPrice;
  const net = round2(gross * (1 - line.lineDiscountPct / 100));
  const vat =
...
export const round2Money = round2;
